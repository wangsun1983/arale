...
void split_path(const char *path,char *array[])
{
    ...
    while(cursor < length)
    {
        if(path[cursor] == '/')
        {
            if(cursor - start != 0)
            {
                 ...
                 //LOGD("split_path p1 is %s ,cursor is %d,start is %d\n",p1,cursor,start);
                 array[count] = p1;
                 count++;
                 start = cursor++;
            }
    ...
    if(start < length)
    {
        ...
        //LOGD("split_path p2 is %s ,cursor is %d,start is %d\n",p1,cursor,start);
        array[count] = p1;
    }
}
...
addr_t file_path_match(const char *pathname,partition_data **patition,inode **up_node,inode **select_node)
{
    ...
    struct list_head *p;
    //LOGD("dir_create trace1 pathname is %s,length is %d \n",pathname,length);
    list_for_each(p,&partition_list) {
        ...
        *up_node = parent;
        *patition = find_part;
        //LOGD("table file name is %s,root is %s find_index is %d,length is %d\n",
        //         inode_table[0].file.name,root,find_index,length);

        if(inode_table != NULL
           && kstrcmp(inode_table[0].file.name,root) == 0)
        {
            //LOGD("find hit,inode_table[0] is %s,root is %s \n",inode_table[0].file.name,root);
            //LOGD("find_index is %d,length is %d ",find_index,length);
            find_index++;
            ...
            list_for_each(dir_node,&parent->child_list){
                ...
                //LOGD("find_node->file.name is %s,path is %s \n",find_node->file.name,path[length - 1]);
